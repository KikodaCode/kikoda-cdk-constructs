...
import { emptyDir, readdir, readFile } from 'fs-extra';
import { Connection, createConnection } from 'promise-mysql';
import { Extract } from 'unzipper';
import { DatabaseMigratorCustomResourceProps } from '../database-migrator';
...
const connectToDatabase = async (connectionDetails: ConnectionDetails): Promise<Connection> => {
...
const runMigrationScripts = async (connection: Connection) => {
  console.log('Listing & sorting migration scripts to run');

  const fileNames = (await readdir(scriptPath, { withFileTypes: true }))
    .filter(x => x.isFile())
    .map(x => x.name)
    .sort();
  ...
  for (const fileName of fileNames) {
    ...
    const fullFilePath = join(scriptPath, fileName);

    const sql = await readFile(fullFilePath, 'utf8');

    await connection.query(sql);
  }
};

const AsyncCreate = async (params: DatabaseMigratorCustomResourceProps) => {
...
const AsyncUpdate = async (
  _physicalId: string,
  params: DatabaseMigratorCustomResourceProps,
  // oldparams: DatabaseMigratorCustomResourceProps,
) => {
