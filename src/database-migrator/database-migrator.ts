...
export interface MySqlDatabaseMigratorCustomResourceProps {
...
export interface MySqlDatabaseMigratorProps {
  /**
...
const resourceType = 'Custom::CDKMySqlDatabaseMigrator';

export class MySqlDatabaseMigrator extends Construct {
  constructor(scope: Construct, id: string, props: MySqlDatabaseMigratorProps) {
    super(scope, id);
    ...
    const handler = new TypescriptSingletonFunction(this, 'CustomResourceHandler', {
      ...
      entry: handlerFile,
      handler: 'main',
      depsLockFilePath: path.join(__dirname, './handlers/yarn.lock'),
    ...
    // custom resource
    const customResourceUniqueId = `CDKMySqlDatabaseMigrator${props.vpc.node.addr}`;
    ...
    const customResource = new CustomResource(this, customResourceUniqueId, {
      ...
      } as MySqlDatabaseMigratorCustomResourceProps,
    });
  ...
  private zipScriptFiles(scriptPath: string) {
    ...
    zip.writeZip(path.join(tempZipSqlDirectory, tempZipSqlFileName), err => {
      if (err) {
        console.log(err);
        throw err;
      }
