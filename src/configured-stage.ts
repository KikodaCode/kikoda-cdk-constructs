...
const CONFIGURED_STAGE_SYMBOL = Symbol.for('@kikoda/cdk-constructs.ConfiguredStage');
 ...
 *
 * @typeParam TConfig - a generic type that represents the configuration for the stage.
 * @extends {Stage}
 */
export class ConfiguredStage<TConfig> extends Stage {
  /**
   * Return the `ConfiguredStage` this construct is contained with, if
   * available. If called on a nested stage, returns its parent. This method is
  ...
  /**
   * Configured Stage construct to be used with the Deployment Pipelines
   * construct. This stage allows for use of the specified generic type to be
   * made available as the config property.
   *
   * @constructor
