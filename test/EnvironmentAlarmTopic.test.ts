...
import {
  AlarmLevels,
  EnvironmentAlarmTopic,
  EnvironmentAlarmTopicProps,
} from "../src";

describe("EnvironmentAlarmTopic", () => {
  class TopicStack extends Stack {
    public alarm: EnvironmentAlarmTopic;
    constructor(props: EnvironmentAlarmTopicProps) {
      super();
      this.alarm = new EnvironmentAlarmTopic(this, "test", props);
    }
  }
  test("has topic", () => {
  ...
  `(
    "when level is: $level display name and topic name contain $level",
    ({ level }) => {
      const topicsStack = new TopicStack({
        level,
      });
  ...
  test.each`
    ...
    ${"1a2B3%"}
  `(
    "when prefix is: $prefix display name and topic name contain $prefix",
    (prefix) => {
      const topicsStack = new TopicStack({
        level: AlarmLevels.INFO,
        prefix,
